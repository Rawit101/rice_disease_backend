...
function buildLocalKnowledgeFallback(question, diseaseHint = '') {
    ...
    if (!context) {
        return "ขออภัยค่ะ ตอนนี้ระบบ AI มีผู้ใช้งานหนาแน่นมาก เลยตอบแบบ AI ไม่ได้ชั่วคราว\nลองถามชื่อโรคข้าวให้ชัดขึ้น เช่น โรคไหม้ โรคขอบใบแห้ง หรือโรคกาบใบแห้ง แล้วหนูจะดึงข้อมูลจากไฟล์กรมการข้าวในเครื่องมาตอบให้ค่ะ"
    }
    ...
    return `ตอนนี้ระบบ AI มีผู้ใช้งานหนาแน่นมาก หนูตอบจากไฟล์ข้อมูลกรมการข้าวในเครื่องให้ก่อนนะคะ\n\n${title}\n\n${shortContent}\n\nแหล่งข้อมูล: ${source}`
}
...
// =====================
// ตรวจจับภาษาอังกฤษใน AI response — ถ้ามีเยอะเกินไปให้ถือว่า AI ตอบผิดภาษา
// =====================
function containsTooMuchEnglish(text) {
    // นับตัวอักษร a-z, A-Z (ไม่รวมตัวเลข, emoji, สัญลักษณ์)
    const englishChars = (text.match(/[a-zA-Z]/g) || []).length
    const thaiChars = (text.match(/[\u0E00-\u0E7F]/g) || []).length
    const totalChars = englishChars + thaiChars
    if (totalChars === 0) return false
    const englishRatio = englishChars / totalChars
    // ถ้าตัวอักษร English มากกว่า 30% ของข้อความ ถือว่ามี English เยอะเกินไป
    return englishRatio > 0.30
}
...
function getLocalReply(text) {
    ...
    if (GREETING_PATTERNS.test(trimmed)) {
        return "🌾 สวัสดีค่ะ! หนู \"น้องข้าวสวย\" ผู้เชี่ยวชาญเรื่องข้าว\n\n📸 ส่งรูปใบข้าวมาวิเคราะห์โรคได้เลย\n💬 หรือถามเรื่องข้าวมาได้เลยค่ะ!"
    }
    if (FAREWELL_PATTERNS.test(trimmed)) {
        return "👋 ลาก่อนค่ะ! ถ้ามีปัญหาเรื่องข้าว กลับมาถามได้ตลอดนะคะ 🌾"
    }
    if (THANKS_PATTERNS.test(trimmed)) {
        return "😊 ยินดีค่ะ! ถ้ามีอะไรอยากถามเพิ่มเติมเรื่องข้าว บอกได้เลยนะคะ 🌾"
    }
...
2. ห้ามถามกลับว่า "คุณถามเรื่องโรคอะไร" หรือ "ใช่ไหมคะ" เด็ดขาด ให้ตอบเลยทันที
...
5. ถ้า user ถามรายละเอียดยา/สาร ให้ลงลึก เช่น ยี่ห้อ วิธีผสม ช่วงเวลาพ่น ข้อควรระวัง`
            ...
            if (session.chatHistory && session.chatHistory.length > 0) {
                const historyText = session.chatHistory
                    .map(h => h.role === 'user' ? `User: ${h.text}` : `น้องข้าวสวย: ${h.text}`)
                    .join('\n')
                contextInfo += `\n\nประวัติสนทนาล่าสุด:\n${historyText}\n\nกฎ: ใช้ประวัตินี้เป็น context ตอบต่อเนื่องจากบทสนทนาเดิม ไม่ต้องทักทายซ้ำ ไม่ต้องถามยืนยัน ตอบเลย`
            }
        }

        const systemPrompt = `[ภาษา] ตอบเป็นภาษาไทยเท่านั้น ห้ามใช้ภาษาอังกฤษทุกกรณี ห้ามใส่ชื่อภาษาอังกฤษในวงเล็บ ห้ามแปลเป็นภาษาอังกฤษ ห้ามมีคำภาษาอังกฤษแม้แต่คำเดียว ถ้าเป็นชื่อสารเคมีให้ทับศัพท์เป็นภาษาไทยเท่านั้น

[บทบาท] คุณคือผู้เชี่ยวชาญด้านโรคข้าวและการดูแลข้าวโดยเฉพาะ ชื่อ "น้องข้าวสวย" เป็นผู้หญิง ใช้สรรพนาม "หนู" แทนตัวเอง ใช้ "ค่ะ/คะ/นะคะ" แทน "ครับ/นะครับ" ทำหน้าที่ให้คำปรึกษาเกษตรกรชาวนาไทย อ้างอิงองค์ความรู้จากกรมการข้าว ห้ามตอบคำถามที่ไม่เกี่ยวข้องกับข้าวเด็ดขาด
...
2. ถ้าไม่เกี่ยวกับข้าว → ตอบว่า "ขออภัยค่ะ หนูตอบได้เฉพาะเรื่องข้าวเท่านั้นนะคะ ลองถามเรื่องข้าวมาได้เลย 🌾"
3. ห้ามใช้ ** หรือ markdown
4. ถ้า user ถามสั้นๆ เช่น "วิธีแก้" "ทำยังไง" "ใช้ยาอะไร" ให้ตอบโดยอ้างอิงโรคที่วิเคราะห์ล่าสุด
5. ห้ามใช้ภาษาอังกฤษเด็ดขาด ทุกคำต้องเป็นภาษาไทยเท่านั้น
...
- ภาษาไทยเท่านั้น ห้ามมีภาษาอังกฤษใดๆ ทั้งสิ้น ง่ายๆ เป็นกันเอง
- ระบุชื่อสาร อัตราส่วน วิธีใช้ ให้ชาวนาใช้ได้จริง
- ย้ำอีกครั้ง: ห้ามมีภาษาอังกฤษแม้แต่คำเดียว${contextInfo}`

        let fullText = await callAI(text, { systemPrompt })

        // ตรวจสอบว่า AI ตอบเป็นภาษาไทยจริงหรือไม่ — ถ้าเป็น English ให้ใช้ fallback
        if (containsTooMuchEnglish(fullText)) {
            log('WARN', '⚠️ Chatbot responded in English, using fallback')
            fullText = buildLocalKnowledgeFallback(text, session?.disease || '')
        }
        ...
        return "❌ ขออภัยค่ะ เกิดข้อผิดพลาด\nกรุณาลองใหม่อีกครั้งนะคะ"
    }
}
...
📸 พบอาการผิดปกติ ส่งรูปมาใหม่ได้เลยนะคะ`
    }
    ...
    function findFallback(diseaseName) {
        ...
        return fallbackAdvice['ไม่พบโรค']
    }

    // ดึงข้อมูลจาก RAG/กรมการข้าว เพื่อให้ AI ใช้อ้างอิง
    const knowledgeContext = await getKnowledgeContext(disease, disease)

    const systemPrompt = `[ภาษา] ตอบเป็นภาษาไทยเท่านั้น ห้ามใช้ภาษาอังกฤษทุกกรณี ห้ามใส่ชื่อภาษาอังกฤษในวงเล็บ ห้ามแปลเป็นภาษาอังกฤษ ห้ามมีคำภาษาอังกฤษแม้แต่คำเดียว ถ้าเป็นชื่อสารเคมีให้ทับศัพท์เป็นภาษาไทยเท่านั้น

[บทบาท] คุณคือผู้เชี่ยวชาญด้านโรคข้าว ทำหน้าที่ให้คำแนะนำเกษตรกรชาวนาไทยเกี่ยวกับโรคข้าวที่ตรวจพบ

[ข้อมูลอ้างอิงจากกรมการข้าว]
${knowledgeContext || 'ไม่พบข้อมูลอ้างอิง ให้ตอบจากความรู้ของคุณเองแบบผู้เชี่ยวชาญ'}

[รูปแบบการตอบ] ตอบตาม format นี้เท่านั้น ห้ามใช้ ** หรือ markdown:

📌 อาการ
(อธิบายสั้นๆ 1-2 บรรทัด เป็นภาษาไทย อ้างอิงข้อมูลจากกรมการข้าว)

💊 วิธีแก้
- ข้อ 1 (ระบุชื่อสาร/วิธีการเป็นภาษาไทย + อัตราส่วนหรือวิธีใช้สั้นๆ)
- ข้อ 2 (ระบุชื่อสาร/วิธีการเป็นภาษาไทย + อัตราส่วนหรือวิธีใช้สั้นๆ)
- ข้อ 3 (ระบุชื่อสาร/วิธีการเป็นภาษาไทย + อัตราส่วนหรือวิธีใช้สั้นๆ)

⚠️ ระวัง: (1-2 ประโยค เป็นภาษาไทย)

รวมไม่เกิน 120 คำ ภาษาไทยง่ายๆ เป็นกันเอง ให้ข้อมูลที่ชาวนาใช้ได้จริง ย้ำอีกครั้ง: ห้ามมีภาษาอังกฤษแม้แต่คำเดียว ให้อ้างอิงข้อมูลจากกรมการข้าวเป็นหลัก`

    const userMessage = `ให้คำแนะนำเป็นภาษาไทยเท่านั้นสำหรับโรคข้าวนี้:\nชื่อโรค: ${disease}\nระดับความรุนแรง: ${severity}`

    try {
        const aiResponse = await callAI(userMessage, { systemPrompt })
        // ตรวจสอบว่า AI ตอบเป็นภาษาไทยจริงหรือไม่
        if (containsTooMuchEnglish(aiResponse)) {
            log('WARN', '⚠️ AI responded in English, using fallback advice')
    ...
    try {
        ...
        if (response.data && response.data.predictions && response.data.predictions.length > 0) {
            ...
            const diseaseMap = {
                ...
                'rice_blast': 'โรคไหม้',
                'leaf_blast': 'โรคใบไหม้',
                'narrow_brown_spot': 'โรคใบขีดสีน้ำตาล',
                'narrow_brown_leaf_spot': 'โรคใบขีดสีน้ำตาล',
...
📸 ส่งรูปใบข้าวมาวิเคราะห์โรคได้เลยค่ะ!`
            }])
            break
...
📸 ส่งรูปใบข้าวมาวิเคราะห์โรคได้เลย!`
        ...
        default:
            await replyMessage(replyToken, [{
                type: "text",
                text: "🌾 สวัสดีค่ะ! หนู \"น้องข้าวสวย\" ผู้เชี่ยวชาญเรื่องข้าว\n📸 ส่งรูปใบข้าวมาวิเคราะห์โรคได้เลยค่ะ!"
            }])
    }
}
    ...
    try {
        ...
        if (result.disease === "ไม่พบโรค" || result.confidence === 0) {
            await pushMessage(userId, [
                {
                    type: "text",
                    text: `✅ ไม่พบร่องรอยของโรคข้าวที่ AI รู้จักในภาพนี้ค่ะ\n(หรืออาจจะถ่ายไกล/ไม่ชัดไป)\n\n📸 ลองถ่ายรูปใหม่นะคะ:\n- ถ่ายใกล้ๆ ใบที่มีอาการ\n- ใช้แสงให้ดี\n- ให้ภาพชัด\n- ลองอีกมุม`
                }
            ])
            return
        }
